...
# macOS builds
build-darwin-amd64:
	# For GitHub Actions, we'll use CGO_ENABLED=1 with proper cross-compiler
...
build-windows-arm64:
	# For GitHub Actions, we'll use CGO_ENABLED=1 with proper cross-compiler
	GOOS='windows' GOARCH='arm64' CGO_ENABLED=1 go build -ldflags="-X xel/shared.RuntimeVersion=$(VERSION) -X xel/shared.EngineVersion=$(ENGINE_VERSION)" -o ./bin/xel-windows-arm64.exe
